...
class MainMenu(Menu):
    ...
    def display_menu(self):
        ...
        while self.run_display:
            ...
            self.game.draw_text('Crossing Game', 20, self.game.BLACK,
                                self.game.DISPLAY_W/2, self.game.DISPLAY_H/3 - 40)
            ...
            if self.state == 'Start':
                self.game.display.blit(
                    self.start_hover_img, (self.startx, self.starty))
            else:
                self.game.display.blit(
                    self.start_img, (self.startx, self.starty))

            if self.state == 'About':
                self.game.display.blit(
                    self.about_hover_img, (self.aboutx, self.abouty))
            else:
                self.game.display.blit(
                    self.about_img, (self.aboutx, self.abouty))

            if self.state == 'Exit':
                self.game.display.blit(
                    self.exit_hover_img, (self.exitx, self.exity))
            else:
